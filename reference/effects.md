...
| Text | `E_TEXTCTRL_Text=WORDS,E_CHOICE_Text_Dir=left\|wavey\|none,E_TEXTCTRL_Text_Speed=12,E_FONTPICKER_Text_Font='Family' 18` | Speed is a TEXTCTRL key; `E_SLIDER_Text_Speed` is silently ignored and every line then scrolls at the default rate (10). Display fonts with only a Regular face smear when asked for bold |
