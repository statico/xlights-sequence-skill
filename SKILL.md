...
They name timestamps ("the drop at 0:55", "the props fade from 0 % at 1:52", "the dragster teleports"). Map each to the table in [reference/beats-and-hits.md](reference/beats-and-hits.md) (symptom -> cause). Marks they draw on a `Cues` timing track are kept across regeneration (`S.apply_cues`); a split in their `Sections` track is a marker: onset-check the mp3 there and put the hit at the marker. Anything they type on a `Notes` track is kept verbatim and printed with its time and beat index by `scripts/notes.py` - tell them to click the circle beside `Notes` first (it is the active-timing-track selector, not an enable toggle: exactly one track is active, `t` adds only to it, and the vertical gridlines across the model rows are that track's marks), then hit `t` and **shift+double-click** the mark to type its label (plain double-click plays it unless `Edit > Preferences > Effects Grid > Double Click Mode` is set to "Edit Text"; shift inverts whichever mode is set), so a complaint arrives pinned to the grid. There is no keybinding to delete a `t` mark: select the timing effect between two marks in that row and hit Delete. If they edited text speed/start/end in the GUI, `scripts/xsq_dump.py` lists their values so you can fold them into the generator before the next run. xLights won't reload an open sequence: File > Close Sequence (No) then Open.
