...
They name timestamps ("the drop at 0:55", "the props fade from 0 % at 1:52", "the dragster teleports"). Map each to the table in [reference/beats-and-hits.md](reference/beats-and-hits.md) (symptom -> cause). Marks they draw on a `Cues` timing track are kept across regeneration (`S.apply_cues`); a split in their `Sections` track is a marker: onset-check the mp3 there and put the hit at the marker. Anything they type on a `Notes` track is kept verbatim and printed with its time and beat index by `scripts/notes.py` - tell them to hit `t` on that track and double-click the mark to type (the Effects Grid "Double Click Mode" must be "Edit Text"), so a complaint arrives pinned to the grid. There is no keybinding to delete a `t` mark: uncheck the radio circle beside the track name, select the timing effect between two marks, Delete. If they edited text speed/start/end in the GUI, `scripts/xsq_dump.py` lists their values so you can fold them into the generator before the next run. xLights won't reload an open sequence: File > Close Sequence (No) then Open.
