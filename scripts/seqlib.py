...
class Song:
    ...
    def timings(self,sections,words):
        ...
        t["Notes"]=self.load_track("Notes")   # free-text notes the user types on a mark: kept verbatim, never interpreted (scripts/notes.py reads them)
        return t
    def load_track(self,name):
        """The marks on a timing track in the file we are about to overwrite, as [(start_ms, end_ms, label)]."""
        if not os.path.exists(self.out): return []
        m=re.search(rf'<Element type="timing" name="{name}">(.*?)</Element>',open(self.out).read(),re.S)
        if not m: return []
        un=lambda t: t.replace("&lt;","<").replace("&gt;",">").replace("&quot;",'"').replace("&amp;","&")
        return [(int(a),int(b),un(l)) for l,a,b in re.findall(r'<Effect label="([^"]*)" startTime="(\d+)" endTime="(\d+)"',m.group(1))]
    def load_cues(self):
        ...
        self.cues=[]
        if not os.path.exists(self.out): return []
        self.cues=self.load_track("Cues")
    ...
    def write(self,timings,script,extra=""):
        ...
        esc=lambda t: t.replace("&","&amp;").replace("<","&lt;").replace(">","&gt;").replace('"',"&quot;")   # xLights does not sanitise a label you type, and a quote in one would break the xsq on the next run
        def timing(name,evs): return f'    <Element type="timing" name="{name}"><EffectLayer>'+"".join(f'<Effect label="{esc(l)}" startTime="{a}" endTime="{b}"/>' for a,b,l in evs)+"</EffectLayer></Element>"+NL
